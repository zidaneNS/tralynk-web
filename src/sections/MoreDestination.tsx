...
export default function MoreDestination() {
  return (
    <div className="flex flex-col gap-y-8 items-center w-full px-4 py-12">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 w-full md:px-8">
        {moreDestinations.map((dest, idx) => (
          <div key={idx} className="rounded-md w-full h-[75vh] bg-cover bg-center group" style={{ backgroundImage: `url(${dest.imgSrc})` }}>
            <div className="w-full flex flex-col gap-y-6 justify-end p-8 h-full bg-linear-to-t from-black/80 to-black/40 text-white rounded-md">
