...
export default function Home() {
  return (
    <div className="flex flex-col items-center justify-center md:justify-end h-screen bg-black/40 py-12 md:py-32 gap-y-8 md:gap-y-12 text-white">
      <div className="flex gap-x-3 items-center">
        {Array.from({ length: 5 }).map((_, idx) => (
          <IoStar key={idx} className="size-4 md:size-6" />
        ))}
      ...
      <div className="flex flex-col md:flex-row md:justify-center gap-6 items-center">
        <button className="bg-white text-black py-2 px-6 rounded-full cursor-pointer hover:bg-yellow-600 hover:text-white duration-300 text-sm">Book your Tour</button>
